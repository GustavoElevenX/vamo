...
const STATIC_REWARDS: Reward[] = [
  ...
  {
    id: 'almoco-lider',
    title: 'Almoço com a Liderança',
    description: 'Almoço exclusivo com o CEO ou gestor para conversar sobre carreira e estratégia.',
    cost_xp: 2000,
    icon: Utensils,
    category: 'reconhecimento',
  ...
  },
  {
    ...
    description: 'Crédito de R$ 100 em cartão presente para usar como preferir.',
    cost_xp: 3000,
    icon: Gift,
    category: 'financeiro',
  ...
  },
  {
    ...
    description: 'Inscrição em curso, workshop ou evento da sua escolha. Invista no seu crescimento.',
    cost_xp: 4000,
    icon: BookOpen,
  ...
  },
  {
    ...
    description: 'Melhoria no seu setup de trabalho: headset, cadeira, monitor ou acessório à escolha.',
    cost_xp: 8000,
    icon: Monitor,
    category: 'desenvolvimento',
    ...
    <div className="space-y-6">
      ...
      {userXp && (
        <div className="flex items-center gap-2 rounded-lg border border-border/40 bg-accent/20 px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground">
            Você está no <strong>Nível {userXp.current_level}</strong> com {availableXp.toLocaleString('pt-BR')} XP acumulados.
      ...
      <div className="rounded-lg border border-border/40 bg-muted/30 p-4">
        <div className="flex items-start gap-3">
          ...
          <p className="text-xs text-muted-foreground">
            Os resgates são solicitações enviadas ao seu gestor para aprovação. O XP
            é descontado somente após a confirmação. Novas recompensas são adicionadas
            periodicamente conforme políticas da empresa.
          </p>
        </div>
      </div>
    </div>
