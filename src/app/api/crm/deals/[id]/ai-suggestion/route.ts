...
export async function POST(_request: Request, { params }: Params) {
  try {
    ...
    if (error || !deal) return NextResponse.json({ error: 'Deal nao encontrado' }, { status: 404 })
    ...
    const { data } = await callOpenAIJSON<Suggestion>({
      systemPrompt: 'Voce e VAMO IA, assistente de vendas consultivo. Analise o deal e sugira o proximo passo mais importante em 1-2 frases. Seja direto, especifico e acionavel. Responda em portugues. Retorne JSON: { "suggestion": string, "urgency": "low" | "medium" | "high" }',
    ...
    return NextResponse.json({
      suggestion: data.suggestion || 'Revise o historico do deal e combine um proximo passo claro com data.',
      urgency: ['low', 'medium', 'high'].includes(data.urgency) ? data.urgency : 'medium',
    })
  } catch (error) {
    console.error('POST ai-suggestion', error)
    return NextResponse.json({
      suggestion: 'Revise o historico do deal e combine um proximo passo claro com data.',
      urgency: 'medium',
    })
  }
}
